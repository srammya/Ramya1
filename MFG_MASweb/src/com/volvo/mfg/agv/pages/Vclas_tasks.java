...
public class Vclas_tasks extends DriverSuite {
	...
	public void Vclas_getAGVTask() throws InterruptedException {
 			...
 			if (driver
 				...
 				for (int i = 2; i <= tableRows.size(); i++) {
 					...
 					String objTaskRow = objectTableData + "/table/tbody/tr[1]/td[2]/table/tbody/tr";
 					 ...
 					 tableTaskRows = driver.findElements(By.xpath(objTaskRow));

 					for (k = 0; k <= tableTaskRows.size() - 1; k++) {
 		...
 		if (iTemp == 30) {
 			...
 			Assert.assertEquals(tableTaskRows.get(k).findElements(By.tagName("td")).get(0).getText().toLowerCase(), tdrow.get("FID"));
